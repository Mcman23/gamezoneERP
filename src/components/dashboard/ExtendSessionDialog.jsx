...
export default function ExtendSessionDialog({ open, onOpenChange, table, session, onConfirm }) {
  ...
  const extraCost = ((extraMinutes / 60) * table.hourly_rate).toFixed(2);
      ...
      <DialogContent className="bg-card border-border max-w-sm">
        ...
        <div className="space-y-4 py-2">
          <div className="flex gap-2">
            {[15, 30, 60, 90].map(t => (
              <Button key={t} size="sm" variant={extraMinutes === t ? "default" : "outline"} onClick={() => setExtraMinutes(t)} className="text-xs flex-1">
                {t} dəq
              </Button>
            ))}
          </div>
          ...
          <div className="bg-primary/5 border border-primary/20 rounded-xl p-4">
            <div className="flex justify-between">
              ...
              <span className="font-bold text-primary text-lg">{extraCost} ₼</span>
            </div>
          </div>
        </div>
