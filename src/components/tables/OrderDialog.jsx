...
import { Plus, Minus, ShoppingCart, Coffee, GlassWater, Cookie, Package } from 'lucide-react';
...
export default function OrderDialog({ open, onOpenChange, table, session, onConfirm }) {
  ...
  const handleConfirm = () => {
    const items = Object.values(cart).map(item => ({
      ...
      quantity: item.quantity, unit_price: item.product.price, total_price: item.product.price * item.quantity,
    }));
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!v) setCart({}); onOpenChange(v); }}>
      <DialogContent className="bg-card border-border max-w-lg max-h-[90vh] flex flex-col">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-foreground"><ShoppingCart className="w-5 h-5 text-primary" /> {table.name} — Sifariş</DialogTitle>
        </DialogHeader>
        ...
        <ScrollArea className="flex-1 max-h-[300px]">
          <div className="grid grid-cols-2 gap-2 pr-2">
            {filteredProducts.map(product => {
              ...
              return (
                <div key={product.id} className={cn("rounded-lg border p-3 cursor-pointer transition-all hover:border-primary/30", inCart > 0 ? "border-primary/30 bg-primary/5" : "border-border")} onClick={() => addToCart(product)}>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {inCart > 0 && (
                      <div className="flex items-center gap-1.5">
                        <Button size="icon" variant="outline" className="h-6 w-6" onClick={(e) => { e.stopPropagation(); removeFromCart(product.id); }}><Minus className="w-3 h-3" /></Button>
                        <span className="text-sm font-bold w-5 text-center">{inCart}</span>
                        <Button size="icon" variant="outline" className="h-6 w-6" onClick={(e) => { e.stopPropagation(); addToCart(product); }}><Plus className="w-3 h-3" /></Button>
                      </div>
                    )}
                  </div>
                </div>
        ...
        {cartCount > 0 && (
          <div className="bg-primary/5 border border-primary/20 rounded-xl p-3 space-y-1.5">
            ...
            <div className="border-t border-border pt-1.5 flex justify-between">
              <span className="font-semibold text-sm">Cəmi</span>
